...
define CONVERT_LATEST_VPD_TO_FSDB
	...
	if [ -z "$$vpd" ]; then \
	  echo "ERROR: No .vpd found in $(SIM_RUNDIR). Your testbench should call \$$vcdpluson; and the sim must run to completion."; \
...
$(asm_output_dir)/%.vpd: $(tests_asm_dir)/%.hex  $(simv)
	...
	"$(VPD2FSDB)" $@ -o $(patsubst %.vpd,%.fsdb,$@)
	@echo "FSDB generated: $(patsubst %.vpd,%.fsdb,$@)"
